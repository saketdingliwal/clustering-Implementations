	...
	*/
	...
	public:
		...
		*/
		...
		KDTree(const std::vector<PointT>& points,int dimm) : root_(nullptr) { build(points);dim = dimm;}
		...
		*/
		std::vector<int> radiusSearch(const PointT& query, double radius, int ext_dim) const
		{
			std::vector<int> indices;
			radiusSearchRecursive(query, root_, indices, radius, ext_dim);
			return indices;
		}
	...
	private:
		...
		*/
		...
		static double distance(const PointT& p, const PointT& q,int ext_dim)
		{
			double dist = 0;
		...
		*/
		void nnSearchRecursive(const PointT& query, const Node* node, int *guess, double *minDist, int ext_dim) const
		{
			if (node == nullptr)
				return;
			...
			const double dist = distance(query, train, ext_dim);
			if (dist < *minDist)
			{
				*minDist = dist;
				*guess = node->idx;
		...
		*/
		void knnSearchRecursive(const PointT& query, const Node* node, KnnQueue& queue, int k,int ext_dim) const
		{
			if (node == nullptr)
				return;
		...
		/** @brief Searches neighbors within radius.
		*/
		void radiusSearchRecursive(const PointT& query, const Node* node, std::vector<int>& indices, double radius, int ext_dim) const
		{
			if (node == nullptr)
				return;
			...
			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;
			radiusSearchRecursive(query, node->next[dir], indices, radius, ext_dim);
